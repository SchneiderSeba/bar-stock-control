...
public class DemoDataConfig {
    ...
    private void product(String sku, String n, String c, String u, String st, String min, String cost, String sell, Supplier s, ProductRepository r) { Product p = new Product(); p.setSku(sku); p.setName(n); p.setCategory(c); if(sku.equals("SPIR-001"))p.setVolumeMl(700); p.setUnit(u); p.setKegSizeLitres("keg".equals(u) ? 50 : null); BigDecimal factor = "keg".equals(u) ? BigDecimal.valueOf(50) : BigDecimal.ONE; p.setStock(new BigDecimal(st).multiply(factor)); p.setMinimumStock(new BigDecimal(min).multiply(factor)); p.setCostPrice(new BigDecimal(cost)); p.setSellingPrice(new BigDecimal(sell)); ProductSupplierSku link=new ProductSupplierSku(); link.setSupplier(s); link.setSku(sku); p.replaceSupplierSkus(java.util.List.of(link)); r.save(p); }
}
