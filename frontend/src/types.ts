...
export type MonthMetrics={startDate:string;endDate:string;sales:number;purchases:number;profit:number;appliedReportCount:number}
export type DashboardPeriod='DAY'|'WEEK'|'MONTH'
export type PeriodComparison={previous:MonthMetrics;current:MonthMetrics}
export type DashboardData={stockValue:number;potentialRevenue:number;potentialProfit:number;purchases:number;productCount:number;lowStockCount:number;monthlyComparison:PeriodComparison;comparisons:Record<DashboardPeriod,PeriodComparison>}
export type SalesReport={id:number;fileName:string;period:'DAILY'|'WEEKLY'|'MONTHLY';startDate:string;endDate:string;status:'READY'|'REJECTED'|'APPLIED';uploadedAt:string;appliedAt?:string;rowCount:number;productCount:number;lines:{productId:number;productName:string;sku:string;sold:number;soldMl:number;stockDecrease:number;stockUnit:string;revenuePricePerStockUnit?:number}[];errors:string[]}
