import type {StockMovement,SalesReport,DashboardData,Invoice,Product,Supplier,User} from './types'
...
export const api={productMovements:(id:number)=>request<StockMovement[]>(`/products/${id}/movements`),salesReports:()=>request<SalesReport[]>('/sales-reports'),uploadSalesReport:(body:FormData)=>request<SalesReport>('/sales-reports',{method:'POST',body}),applySalesReport:(id:number)=>request<SalesReport>(`/sales-reports/${id}/apply`,{method:'POST'}),dashboard:()=>request<DashboardData>('/dashboard'),products:()=>request<Product[]>('/products'),suppliers:()=>request<Supplier[]>('/suppliers'),invoices:()=>request<Invoice[]>('/invoices'),createProduct:(body:unknown)=>request<Product>('/products',post(body)),createProductsBulk:(body:unknown)=>request<Product[]>('/products/bulk',post(body)),updateProduct:(id:number,body:unknown)=>request<Product>(`/products/${id}`,{method:'PUT',body:JSON.stringify(body)}),uploadImage:(id:number,file:File)=>{const body=new FormData();body.append('file',file);return request<Product>(`/products/${id}/image`,{method:'POST',body})},deleteImage:(id:number)=>request<Product>(`/products/${id}/image`,{method:'DELETE'}),createSupplier:(body:unknown)=>request<Supplier>('/suppliers',post(body)),updateSupplier:(id:number,body:unknown)=>request<Supplier>(`/suppliers/${id}`,{method:'PUT',body:JSON.stringify(body)}),createInvoice:(body:unknown)=>request<Invoice>('/invoices',post(body)),adjustStock:(id:number,body:unknown)=>request<Product>(`/products/${id}/adjust`,post(body))}
